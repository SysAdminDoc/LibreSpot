...
Use the existing projects as research and as a parts bin. Read their code, take their working patterns, and vendor their code where the license allows. Do not reinvent what a maintained project already solved; do not copy what its license forbids (see "Licenses" below).

## Read these first, in this order

1. `docs/how-spotx-and-spicetify-alter-spotify.md`. The verified reference for how SpotX and Spicetify change the client at the byte and regex level, the theme format, the exposed API surface, the flag mechanisms, the route collision LibreSpot repairs, and the policy boundary. Everything you build sits on top of what this describes.
...
- **Stay on the safe side of the line.** This feature is theming, layout, window-level polish, local UI feature flags, snippets, presets, and health. It must not add, touch, or expose ad blocking, premium or product-state spoofing, telemetry patches, or any binary patch. Those stay where they are (SpotX, pinned, unchanged). The reference doc's policy section explains why: every Spotify takedown targeted exactly those categories and never a theme. A regression test must assert the new catalogs contain no flag, patch, or snippet from the ad, fraud, premium, or telemetry families.
...
## Licenses (this decides what you may vendor)

LibreSpot is MIT. Vendor code only from compatible licenses and keep the attribution block. Everything else is read-only research: study it, reimplement the idea, cite it in the commit body.

SPDX identifiers below were read from each repository's GitHub license metadata on 2026-09-01. Re-check each one the day you vendor from it; a license can change between commits.

- **Vendor freely (MIT or equally permissive), with attribution:** `spicetify/marketplace` (MIT: the runtime scheme injection in `injectColourScheme`, the Dexie storage layer, the `colorExtractor` vibrancy and ColorApi modes, the Theme Dev Tools editor, `resources/snippets.json` as data), `spicetify/spicetify-themes` (MIT: Dribbblish's sidebar and folder-image logic, StarryNight's turntable, the `text` theme's scheme catalog), `catppuccin/spicetify` (MIT: the accent picker built on `Spicetify.React`, palette export as extra `color.ini` keys), `JulienMaille/spicetify-dynamic-theme` (MIT: Vibrant swatch selection by background lightness, the light/dark sentinel), `hoeci/sort-play` (MIT: the pattern for adding columns, context-menu entries, and a settings surface to Spotify's own tracklist), `rxri/spicetify-extensions` (MIT, but take only `wikify`, `songstats`, `phraseToPlaylist`; do not touch `adblockify`), `Konsl/spicetify-extensions` (MIT: waveform seekbar, context switcher), `Theblockbuster1/spicetify-extensions` (MIT: queue duration, cover ambience), `Xndr2/listening-stats` (MIT: local listening tracker without an external backend), `wSoltani/syncify` (MIT: Marketplace state backup and restore), `Comfy-Themes/Spicetify` (WTFPL: in-app settings UI, font loader, panel-width variables, the `wal16` pywal scheme; permissive, still record it in the notices).
- **Read-only, reimplement the idea in your own code:** `spicetify/cli` (LGPL-2.1: `expFeatures.js`, `homeConfig.js`, `sidebarConfig.js`, the wrapper's `Platform` resolution, the custom-app injection patterns; LibreSpot already reimplements the last one in `Repair-SpicetifyCustomAppWiring.ps1`, keep doing that), `sanoojes/spicetify-lucid` (AGPL-3.0: the Material colour engine seeded from artwork, the settings modal, the version-pinned manifest pattern), `theRealPadster/spicetify-hide-podcasts` (GPL-3.0: DOM-stable hiding of podcast and audiobook surfaces), `ohitstom/spicetify-extensions` (GPL-3.0: `sleepTimer`, `quickQueue`, `volumePercentage`, `playbarClock`, `immersiveView`), `alexk218/tagify` (GPL-3.0: tag and rate tracks, bulk tag to playlist), `NMWplays/Liquify` (AGPL-3.0; also 651 KB of `theme.js`, ideas only), `Spikerko/spicy-lyrics` (AGPL-3.0), SpotX (`patches.json` shapes are data; the script is read-only), BlockTheSpot and the Windhawk mod (research only; native hooks are not part of this feature).
- **No usable license on file, treat as all-rights-reserved and reimplement:** `Astromations/Hazy` (custom background upload stored locally), `surfbryce/beautiful-lyrics` (issues disabled), `harbassan/spicetify-apps` (Stats and Library). `Ingan121/WMPotify` carries a non-standard license text (GitHub reports NOASSERTION); read it before taking anything beyond the mini-player and high-contrast ideas.

When in doubt, reimplement. Keep a `THIRD_PARTY_NOTICES.md` next to the vendored code listing every source, license, and commit, and add a test that fails if a vendored file lacks a notices entry.
...
3. **Features.** The curated experimental-feature picker (RD-135). Group the client-side flags by what they change (Playback, Now Playing, Library, Home, Lyrics, Layout, Fun). Show Spotify's own description string for each. Apply through `expFeatureOverride` and `RemoteConfigDebugAPI` live, and persist so they survive restart. Mark server-gated flags (Enhance, quality tiers, Jam) as unavailable and never write them. Never list an ad, fraud, or telemetry flag.
...
- Custom app and engine: a minimal Node workspace under `src/LibreSpot.App/` (or a name that fits the repo) with `vitest` and a DOM (`happy-dom`; keep fixture windows to one per file, the suite has a memory ceiling), tests for scheme parsing and derivation, layer composition, flag allowlist enforcement, snippet catalog validation, the self-test against a fixture bundle with a removed anchor, export round-trips (`color.ini` in, engine state, `color.ini` out, compared against the input, not against a second trip), and a red test that fails if any banned flag or patch family appears. Run the linter explicitly; a green suite does not cover it.
...
2. **Parts bin.** Clone the vendor-allowed repos into a scratch folder, extract the exact pieces you will reuse, record license and commit in `THIRD_PARTY_NOTICES.md`, and reimplement the read-only patterns.
...
5. **Catalogs.** Flags, snippets, presets, built-in themes, the banned-family test.
...
- Every catalog is provably free of ad, fraud, premium, and telemetry entries.
