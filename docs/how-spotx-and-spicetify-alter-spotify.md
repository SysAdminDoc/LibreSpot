...
LibreSpot 4.4.0 installs `CustomApps\librespot` plus the `librespot-engine.js` companion through the same reviewed custom-app path used by the other catalog apps. The custom app owns the Store, Look, Tweaks, Features, Presets, and Health pages under `/librespot/*`. Store is the default catalog for themes, extensions, and apps, with search, category filters, screenshots, and clear descriptions before anything is applied. Look can preview every included theme without saving it. The companion runs on every route, keeps live state in Spicetify local storage, and replaces Spotify's Marketplace shortcut with **LibreSpot Settings**. The old `/librespot/extensions` and `/librespot/marketplace` paths redirect to Store.
