...
LibreSpot 4.1.2 installs `CustomApps\librespot` plus the `librespot-engine.js` companion through the same reviewed custom-app path used by the other catalog apps. The custom app supplies the six-panel `/librespot/*` surface. The companion runs on every route, owns the live state in Spicetify local storage, and adds a guarded top-bar entry if Spotify does not render the normal custom-app navigation link.
