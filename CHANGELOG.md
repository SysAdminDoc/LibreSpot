...
- The README now opens with the failures people actually report when they run SpotX and Spicetify by hand, quoted from the threads, each next to the LibreSpot feature that handles it. Both upstreams said on 2026-09-07 that the two tools should not be combined, which is the thing LibreSpot exists to make work, and the README explained that three hundred lines down. A test resolves every in-page link in that section against a real heading.
