...
- The theme gallery is checked at the window size the README screenshots are taken at, and at the smallest supported window: every card whole, inside the gallery, with no scrollbar of its own. The screenshot that suggested otherwise is older than the fix.
...
- The badge line shows both licences. The hosts are MIT and the in-Spotify engine inside the same executable is AGPL-3.0-only, which the prose said three times while the single badge said MIT.
- The README now opens with failures reported on the upstream trackers, quoted from the threads, each next to the LibreSpot feature that handles it. A Spicetify member said on 2026-09-07 that the two tools should not be used together, which is the thing LibreSpot exists to make work, and the README explained that three hundred lines down. A test resolves every in-page link in that section against a real heading.
- The Store's extension count and the README's no longer look like a contradiction. The Store counts everything the engine knows about, which includes the LibreSpot companion that installs with the engine, so it reads 16 where Custom Install offers 15. The counting rule is written into the catalog schema, the README says it beside the screenshot that shows the higher number, and gates on both sides fail when either count stops following the rule.
- The route re-wiring proof now pins the fixture it runs against. An adversarial review passed the whole check with a 287-byte hand-written stand-in, because nothing tied the fixture to Spotify's bytes. The baseline records the SHA256 of the bundle before and after the repair, so a stand-in is refused before any anchor runs, and the repair has to reproduce the recorded result exactly. The README claim gate reads a build written without backticks, refuses a second claim, and the assertion guarding the stylesheet gate has a positive control after it turned out to be capturing nothing at all.
- The Store card and Custom Install now say when a custom app has open upstream defects. The bundled Stats app has had a rate-limit failure open since December 2025, a rewrite waiting since February, and an install that removes your theme until Spicetify is applied again, and nothing in LibreSpot mentioned any of it. The three issue links live in the catalog, an asset with open issues can never be an Easy-mode default, and `-CatalogTruth` re-reads the issue states so a closed one stops being advertised as still broken.
- The catalog refresh checklist had been carrying a June snapshot that no gate read. Four of its six evaluations named MIT for repositories that are WTFPL, AGPL-3.0-only, or carry no licence file at all, and every recorded last-push date was wrong. All six were re-reviewed against the live repositories on 2026-09-07, the licence rule now describes the download-only posture the product actually ships, and a test fails when an evaluation is older than the pinned Spotify build or disagrees with the licence recorded for that asset in the manifest.
...
- Companion readiness now requires every React, history, local storage, and player method used during bootstrap to be callable. Staged API publication stays in the loading state until the complete surface exists.
- Companion startup now probes routes before registering listeners and rolls back claimed globals, overrides, observers, timers, and listeners when setup fails. An offscreen staged-companion fixture verifies retry after a failed registration, while a stalled background frame probe remains asynchronous.
- Asset-cache objects now stage to flushed temporary files before publication, and index updates re-read under a shared cache lease across the PowerShell and Core lanes. Concurrent saves retain both verified entries, interrupted index writes leave the prior JSON intact, and malformed indexes are retained and reported. Missing, null, or scalar `entries` fields are refused before an import or export can replace the index.
...
- Cache-root and every existing cache or configuration parent reparse point are refused before cache locks, staging, writes, or recovery. Existing files copied into a replacement cache are flushed to disk through the same durable-copy path as imported objects.
...
- The isolated Windows crash fixture now runs with the same compressed win-x64 single-file settings as the release apps, with .NET's `createdump.exe` helper staged beside it. A real `FailFast` run proves the triage flags and streams, two-dump retention, disabled mode, and support-export eligibility. Release staging carries and checksums that helper so the shipped single-file apps can produce the capture they arm.
- Crash capture verification now forces the disabled fixture through a real crash with dump environment variables absent, and both fixture and release publishing pin the exact `RuntimeFrameworkVersion` that supplies their adjacent `createdump.exe` helper.
...
- Public documentation now distinguishes the MIT root hosts from the AGPL-3.0-only in-Spotify app, names the bundled engine and Prism assets, and identifies SpotX, Spicetify CLI, Marketplace, and the theme archive as fetched dependencies. It also names the three user-facing executables plus the adjacent `createdump.exe` helper, uses the full configured Pester suite, and separates raw profile recovery from the complete Health backup.
- Core asset-cache recovery now kills a disposable helper process at each directory publication boundary and verifies the recovered cache and index in a fresh process, including preservation of unindexed files and cleanup of transaction state.
...
- Prism's settings entry now survives a slow client start. The retry waits for Spicetify's menu and modal surfaces, then registers `Spicetify.Menu.Item` and opens `Spicetify.PopupModal.display`. That logic was written months ago and announced as shipped, but it only ever reached a copy of the theme that nothing installs, so the theme people actually had still registered the menu once and gave up. There is one copy of the theme in the tree now.
...
- The UI-automation arguments the shipped executables accept are declared in the release contract and the security policy instead of being left to be discovered. They ship on purpose, because the release binary is what gets measured for startup cost and photographed for the README, and a test proves a smoke run writes nothing outside its own root.
- The Spotify drift check now reads the classmap ceiling from the index spicetify/classmaps publishes, rather than inferring it from folder names. It gets the exact build upstream verified instead of a three-part approximation, stops counting an inherited or legacy classmap as coverage, and no longer spends GitHub API rate limit to find out.
- Two accessibility defects the shell had been carrying: the list of reversible changes in the run activity panel had no accessible name, so a screen reader announced an unlabelled list, and searching Settings for something with no matches left an empty region in the accessibility tree with no bounding box. Both are fixed, and the accessibility scans now cover the overlays, prompts, and error and empty states instead of only the three main workspaces.
...
- Native process output is now bounded at the stream reader, collector queue, retained Spicetify error tail, and redirected worker logs. Oversized lines and dropped batches carry explicit truncation markers, while both streams continue draining so exit codes and failure signatures remain available. Malformed bytes are consumed with replacement decoding so polling keeps moving through later diagnostics.
...
- Support bundles now accept a `.dmp` only when it has the Windows minidump signature and version, a bounded stream directory, and valid data ranges for every stream. If the newest candidate is invalid, LibreSpot skips it and includes the next valid dump instead.
...
- The cross-edition community catalog contract now checks both outputs against the reviewed PowerShell 7 byte hash, so identical drift in both generators cannot hide a format regression.
- CLI Recommended Setup regression coverage now runs install, reapply, repair-install, and missing-config repair paths, then compares the configuration each path writes with the shared catalog defaults.
- Release-notice regression coverage now drives the actual HTTP client through refused, conditional, successful, and malformed responses. It verifies that a refusal reports the status code GitHub returned.
