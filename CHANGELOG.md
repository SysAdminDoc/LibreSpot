...
- Native process output is now bounded at the stream reader, collector queue, retained Spicetify error tail, and redirected worker logs. Oversized lines and dropped batches carry explicit truncation markers, while both streams continue draining so exit codes and failure signatures remain available.
