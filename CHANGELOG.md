...
- Marketplace reset now writes a bounded recovery copy in LibreSpot storage before clipboard access or database deletion. Health can restore, export, or dismiss that copy after the clipboard changes or Spotify starts a fresh runtime, and reset stops when the owned copy cannot be retained.
- Backup restore now snapshots the engine and Marketplace settings before writing. A failed half restores the exact prior keys, removes keys introduced by the failed merge, and retains a bounded recovery copy that names any half it could not compensate.
