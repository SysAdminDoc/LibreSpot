...
- **Spicetify apply failure = auto-restore.** If `backup apply` exits non-zero, the script calls `spicetify restore` to prevent blank screen.
...
## Version History
- v3.1.1 — Reverted broken custom apps/extension packs (v3.2.0). Kept theme preview (async image loading from spicetify-themes repo, in-memory cache).
- v3.1.0 — Audit: anti-hang (-confirm_uninstall_ms_spoti), Spicetify apply failure recovery, 27 lyrics themes, old lyrics/collab icon options, removed dead pre-install code.
