...
|---|---|
| "SpotX-patched client drops xpui-snapshot.js, silently breaking Spicetify custom apps" ([SpotX #892](https://github.com/SpotX-Official/SpotX/issues/892), open) and "Custom app patches silently applied to a bundle the client never loads when xpui-snapshot.js is absent" ([spicetify/cli #3922](https://github.com/spicetify/cli/issues/3922), closed as not planned). Both were opened on 2026-09-07 by one reporter to document the mechanism. A Spicetify member answered "This is SpotX's fault, not ours. SpotX is aware of this, but as far as I know, no fix is planned." and, on the SpotX thread, "We always tell people not to use SpotX & spicetify at the same time because we both modify stuff." SpotX has not replied. | LibreSpot re-wires the custom-app route onto the bundle the patched client actually loads, after Spicetify applies. See [Spotify Compatibility](#spotify-compatibility). |
...
```powershell
gh release verify-asset v4.5.1 .\LibreSpot-Desktop.exe
```
...
```powershell
gh release verify-asset -R SysAdminDoc/LibreSpot v4.5.1 .\LibreSpot-Desktop.exe
```
...
.\Build-Scripts.ps1 -GenerateReleaseManifest -ReleaseRoot .\publish -ReleaseVersion 4.5.1 -ReleaseChannel stable
```
