...
- **In-Spotify backup recovery**, Health snapshots the engine and Marketplace settings before restore. If either write fails, it puts back the exact prior keys, removes keys introduced by the failed merge, and keeps a bounded recovery copy when compensation cannot finish
