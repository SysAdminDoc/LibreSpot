...
public sealed class ReleaseNoticeServiceTests
{
    ...
    public async Task TryGetLatestStableAsync_ASuccessfulBodyIsMappedByTheRealClient()
    {
        ...
        {
            tag_name = "v4.6.0",
            html_url = "https://github.com/SysAdminDoc/LibreSpot/releases/tag/v4.6.0",
            prerelease = false,
            draft = false,
            assets = new[]
            {
        ...
        Assert.Equal("v4.6.0", lookup.TagName);
        Assert.Equal("https://github.com/SysAdminDoc/LibreSpot/releases/tag/v4.6.0", lookup.HtmlUrl);
    ...
    public async Task TryGetLatestStableAsync_MissingMalformedOrDuplicateDesktopDigestFailsClosed()
    {
        var bodies = new[]
        {
            JsonSerializer.Serialize(new { tag_name = "v4.6.0", assets = Array.Empty<object>() }),
            JsonSerializer.Serialize(new { tag_name = "v4.6.0", assets = new[] { new { name = ReleaseNoticeService.DesktopAssetName, digest = "sha256:not-a-hash" } } }),
