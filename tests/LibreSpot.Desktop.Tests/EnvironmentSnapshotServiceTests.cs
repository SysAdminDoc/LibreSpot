...
public sealed class EnvironmentSnapshotServiceTests
{
    ...
    {
        // The drain now lives in ProcessProbe, which both remaining snapshot probes
    ...
    public void SnapshotProbes_ShareOneDrainImplementation()
    {
        ...
        Assert.Equal(2, Regex.Matches(snapshotSource, @"ProcessProbe\.Run\(").Count);
        Assert.DoesNotContain("Get-MpPreference", snapshotSource, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void GetSnapshot_DoesNotTreatEndpointProtectionDefaultsAsAHealthIssue()
    {
        using var fixture = new SnapshotFixture();
        fixture.WriteSpotify(withSpotXMarkers: true);
        ...
        Assert.DoesNotContain(snapshot.HealthReport.Components, component =>
            component.Name.Contains("antivirus", StringComparison.OrdinalIgnoreCase));
    }
