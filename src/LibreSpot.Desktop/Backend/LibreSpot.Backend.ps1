param(
    [ValidateSet('Install', 'CheckUpdates', 'Reapply', 'RepairMarketplace', 'OpenMarketplace', 'ExportMarketplaceState', 'RestoreMarketplaceState', 'SafeMode', 'RestoreSafeMode', 'CreateBackup', 'RestoreBackup', 'RestoreVanilla', 'UninstallSpicetify', 'FullReset', 'RemoveSelfData', 'ClearCache', 'EnableAutoReapply', 'DisableAutoReapply', 'WatchAutoReapply', 'Plan')]
...
function Update-ApplyState {
    ...
    try {
        ...
        if ($Successful) {
            ...
            # A manual reapply is the escape the held Maintenance row offers, so
            # it has to retire the hold. Without this the row would stay up with
            # a stale reason and its own action would look like it did nothing.
            foreach ($entry in (Get-LibreSpotWatcherClearedHoldState).GetEnumerator()) {
...
# Windows PowerShell 5.1 advisories on the download primitive: CVE-2025-54100
# (web-content RCE, CVSS 7.8, December 2025), CVE-2026-26170 (local elevation,
# CVSS 7.8, 2026-04-14) and CVE-2026-40400 (relative path traversal executing
# code over a network, CVSS 8.0, 2026-07-14). SHA256 pinning protects payload
# integrity but no interpreter-side vector, so warn (non-blocking) when the host
...
function Invoke-LibreSpotMaintenance {
    ...
    $patcherReport = Get-ThirdPartyPatcherReport
    if ($patcherReport.HasForeignState -and $Action -in @('Reapply', 'RepairMarketplace', 'RestoreMarketplaceState', 'SafeMode', 'RestoreSafeMode', 'CreateBackup', 'RestoreBackup', 'RestoreVanilla', 'UninstallSpicetify', 'FullReset')) {
    ...
    switch ($Action) {
        ...
        'SafeMode' {
            Update-BackendState -Progress 10 -Status 'Preparing safe mode' -Step 'Snapshotting config and CustomApps'
            $savedConfig = Load-LibreSpotConfig
            Update-BackendState -Progress 45 -Status 'Applying safe mode' -Step 'Disabling extensions and custom apps for one launch'
            Reapply-SavedSpicetifySetup -Config $savedConfig -SafeMode | Out-Null
            Write-Log 'Safe mode is ready. Start Spotify, test the client, then use Restore my setup in Maintenance.' -Level 'SUCCESS'
        }
        'RestoreSafeMode' {
            Update-BackendState -Progress 15 -Status 'Restoring your setup' -Step 'Verifying the safe-mode snapshot'
            $savedConfig = Load-LibreSpotConfig
            Update-BackendState -Progress 50 -Status 'Restoring your setup' -Step 'Restoring config and CustomApps'
            Reapply-SavedSpicetifySetup -Config $savedConfig -RestoreSafeMode | Out-Null
            Write-Log 'The pre-safe-mode setup was restored and reapplied.' -Level 'SUCCESS'
        }
        'CreateBackup' {
...
try {
    ...
    if ($Action -eq 'WatchAutoReapply') {
        ...
        $runWatcherTick = {
            $lease = $null
            try {
                $lease = Enter-LibreSpotMutationLease -Label 'backend watcher reapply'
    ...
    $global:LibreSpotAssetInstallFailures = [System.Collections.Generic.List[object]]::new()
    $mutationLease = $null
    if ($Action -notin @('CheckUpdates', 'OpenMarketplace', 'Plan', 'RemoveSelfData')) {
        $mutationLease = Enter-LibreSpotMutationLease -Label "backend action: $Action"
    }
    $journalWouldChange = ($Action -notin @('CheckUpdates', 'OpenMarketplace', 'WatchAutoReapply', 'Plan'))
    $journalReversible = $Action -in @('SafeMode', 'RestoreSafeMode')
    $journalRollbackHint = if ($journalReversible) { 'Use the safe-mode recovery action backed by the verified local snapshot.' } else { 'Review individual journal entries for action-specific rollback hints.' }
    Start-OperationJournalRun -Action $Action -Target "Backend action: $Action" -WouldChange $journalWouldChange -Reversible $journalReversible -RollbackHint $journalRollbackHint -OperationId $OperationId | Out-Null
    ...
    $message = $_.Exception.Message
    $isMutationBusy = $message -like 'LIBRESPOT_MUTATION_BUSY:*'
    if ($Action -ne 'RemoveSelfData' -and -not $isMutationBusy) {
