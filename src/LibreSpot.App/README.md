...
- `src/extensions/librespot-engine.ts` keeps the engine loaded on every Spotify route and supplies a menu entry when custom-app navigation is unavailable. Companion startup waits for callable React hooks, history navigation, local storage, and player event methods before it uses Spicetify's Menu, Topbar, Platform.History, LocalStorage, and Player surfaces; Prism's own settings entry uses `Spicetify.Menu.Item` and `Spicetify.PopupModal.display`.
- Companion startup has a 30-second API deadline and publishes an accessible error with a retry action. Route probing completes before listeners are attached, and a failed attempt removes its runtime, overrides, observers and listeners. The app binds only after the engine's loaded marker is true, and frame measurement runs after startup with a timeout that defers hidden or stalled samples.
...
- Built-in presets compare their owned settings, so edited profiles and user profiles with matching names do not produce false Applied states.
