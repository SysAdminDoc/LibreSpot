...
# The install work runs in a worker runspace that only receives the globals
# listed in $varNamesForWorker, so a script-scoped root is invisible there and
# the bundled-asset lookup would fall through to powershell.exe's own folder.
# Publish it as a global and export it with the rest.
$global:LibreSpotScriptRoot = $script:ScriptRoot
...
function Test-CompatibilityGate {
    ...
    $issues = @()
    # Only a client past what Spicetify itself declares is worth blocking on.
    # Past LibreSpot's own verified build is a Degraded state, not a broken one,
    # and gating there would be blaming Spicetify for LibreSpot's ceiling again.
    if (-not [string]::IsNullOrWhiteSpace($spicetifyDeclaredMax)) {
        $installedNorm = ($installedVersion -replace '\.(\d+)$','') -replace '^(\d+\.\d+\.\d+).*','$1'
        if (Compare-LibreSpotVersions -Latest $installedNorm -Current $spicetifyDeclaredMax) {
            $issues += "Installed Spotify $installedVersion is newer than Spicetify CLI $($global:PinnedReleases.SpicetifyCLI.Version) declares support for ($spicetifyDeclaredMax). Themes and extensions may not apply correctly."
        }
    }
...
function Update-CompatibilityWarningBadge {
    ...
    if ($warnings.Count -gt 0) {
        ...
        $ui['CompatibilityWarning'].Text = if (Compare-LibreSpotVersions -Latest $spotxTarget.Id -Current $spicetifyDeclaredMax) {
            "⚠ SpotX targets Spotify $($spotxTarget.Id) but Spicetify declares support to $spicetifyDeclaredMax"
        } else {
            "⚠ SpotX targets Spotify $($spotxTarget.Id) but LibreSpot has verified to $verifiedMax"
        }
...
function Test-ForeignPatchWarningIfNeeded {
    ...
    $report = Get-ThirdPartyPatcherReport
    if (-not $report.HasForeignState) { return }
    $script:ForeignPatchWarningShown = $true

    $message = "Spotify at $global:SPOTIFY_EXE_PATH has customization state that LibreSpot did not create.`n`n$($report.Summary)`n`n$($report.Recommendation)"
    try {
...
# Three Windows PowerShell 5.1 advisories reach LibreSpot's download primitive:
# CVE-2025-54100 (web-content remote code execution, CVSS 7.8, fixed December
# 2025), CVE-2026-26170 (local elevation of privilege, CVSS 7.8, fixed 2026-04-14)
# and CVE-2026-40400 (relative path traversal executing code over a network,
# CVSS 8.0, fixed 2026-07-14). Content fetched by Invoke-WebRequest can execute at
# parse time on an unpatched host -- exactly LibreSpot's download pattern. SHA256
# pinning guarantees payload *integrity* but removes none of the interpreter-side
# vectors, so we surface a non-blocking heads-up when the host looks unpatched.
# PowerShell 7+ (Core) is a separate product with its own floor. Pure and
...
$maintBlock = { param($sh,$action)
    ...
    try {
        ...
        if ($action -eq 'CheckUpdates') {
            ...
            $sh.Dispatcher.Invoke([Action]{ $sh.StepLabel.Text="Saving the exact setup before safe mode"; $sh.ProgressBar.Value=30 })
            Reapply-SavedSpicetifySetup -Config $saved -SafeMode | Out-Null
            Write-Log "Safe mode is active. Open LibreSpot Maintenance to restore the verified snapshot." -Level 'SUCCESS'
...
# Package transaction functions are composed from src/powershell/shared before
# the worker runspace is created.
